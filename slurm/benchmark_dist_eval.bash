...
#SBATCH --gres=gpu:2
#SBATCH --partition=gpu_h100  # Adjust this for your cluster
...
if [[ -f "$CONFIG_FILE" ]] && grep -qE '^\s*use_vllm:\s*true' "$CONFIG_FILE"; then
    USE_VLLM="true"
    OPTIONAL_ARGS="$OPTIONAL_ARGS --vllm_mode colocate"
fi
# # if using vllm
# if [[ "$USE_VLLM" == "true" ]]; then
#      srun --nodes=1 --ntasks=1 trl vllm-serve --model $MODEL --revision $REVISION --tensor_parallel_size $TP --data_parallel_size $DP &
...
export CMD=" \
    src/open_r1/eval_model.py --config $CONFIG_FILE $OPTIONAL_ARGS
    "
